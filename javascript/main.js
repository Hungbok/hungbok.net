...
$(document).ready(function() {
    ...
    var navbarHeight = $('.site-header-background').outerHeight(); // 영향을 받을 요소를 선택
    ...
    function hasScrolled() {
        ...
        if (st > lastScrollTop && st > navbarHeight) {
            // Scroll Down
            $('.site-header-background').removeClass('nav-down').addClass('nav-up');
        } else {
            // Scroll Up
            if (st + $(window).height() < $(document).height()) {
                $('.site-header-background').removeClass('nav-up').addClass('nav-down');
            }
        }
