async function loadAsyncScripts() {
    ...
    await loadScript('//www.hungbok.net/javascript/ja/error404.js');
}
...
$(document).ready(function(){
    ...
    if (queryParam) {
        ...
        $.getJSON(`//data.hungbok.net/data/games/${queryParam}.json`, function (data) {
            ...
            function formatDate(date) {
                ...
                switch (parts.length) {
                    case 6: // yyyy-mm-dd-hh-mm-ss
                        formattedDate = `${parts[0]}年${monthName}月${parts[2]}日`;
                        break;
                    ...
                    case 3: // yyyy-mm-dd
                        ...
                        break;
                    case 2: // yyyy-mm
                        formattedDate = `${parts[0]}年${monthName}月`;
                        break;
                    case 1: // yyyy
                        formattedDate = `${parts[0]}年`;
                        break;
                    default:
            ...
            $(".add-on").append('<p class="description-title">このコンテンツは Mod またはアドオンです。</p>'+
            ...
            $(document).ready(function() {
                ...
                $('#age-checking').click(function() {
                    ...
                    if (adjustedAge >= 18) {
                        ...
                        $('body.adult > main > .section').remove();
                        $('#warning').append('<div id="child">申し訳ございませんが、このコンテンツにアクセスすることができません。</div><a class="age-check-back" onclick="window.history.back()">戻る</a>'); // #child 요소 추가
                    }
                });
        ...
        async function loadGameData() {
            try {
                ...
                topGames.forEach(game => {
                    // ja 값이 있으면 그 값을, 없으면 en 값을 사용
                    const title = game.ja ? game.ja.title : game.en.title;
                    const summary = game.ja ? game.ja.summary : game.en.summary;
        ...
        fetch(apiUrl)
            ...
            .then(data => {
                function getLocalizedData(data, key) {
                    if (data['ja'] && key in data['ja'] && data['ja'][key] !== undefined) {
                        return data['ja'][key];
        ...
        .then(data => {
            ...
            const fetchPromises = topGames.map((game, index) => 
                ...
                .then(gameData => {
                    ...
                    const formattedDate = formatDateToJP(date);
                    return {
                        index,
                        gameElement: `
